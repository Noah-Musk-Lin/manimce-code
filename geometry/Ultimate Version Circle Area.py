...
class Ultimate_Version_Circle_Area(Scene):
    def construct(self):
        # 添加文字
        text1 = Text("首先，我们先画一个圆。", font_size=36)
        ...
        # 添加文字
        text2 = Text("然后，我们对圆进行分割。", font_size=36)
        ...
        for division in range(10, max_divisions + 1, 30):
            ...
            if prev_stacked_rectangles is not None:
                self.play(
                    FadeOut(prev_stacked_rectangles, prev_brace, prev_brace_text),
            ...
            self.wait(0.5)
        '''
        text3 = Text("分割次数逐渐增加。", font_size=36)
        ...
        '''
        text3 = VGroup(
            Text("分割次数逐渐增加。", font_size=36),
            Text("圆的面积近似等于这个由矩形堆叠成的类等腰三角形的面积。", font_size=36),
        )
        text3.arrange(DOWN, buff=SMALL_BUFF)
        text3.move_to(UP * 3)  # 将文字移动到圆的上方，可根据需要调整
        self.play(Write(text3), run_time=3) # 显示文字动画
        self.wait(3)  # 保持显示 3 秒
        self.play(FadeOut(text3), run_time=3)
        self.wait(1)
        
        #text4 = Text("圆的面积近似等于这个由矩形堆叠成的类等腰三角形的面积。", font_size=36)
        #text4.move_to(UP * 3)  # 将文字移动到圆的上方，可根据需要调整
        #self.play(Write(text4))  # 显示文字动画
        #self.wait(1)  # 保持显示 1 秒
        #self.play(FadeOut(text4), run_time=1)  # 渐隐动画持续 1 秒
        #self.wait(1)
        '''
        text5 = VGroup(
            Text("这个类等腰三角形的面积为：", font_size=36),
            Tex(r"S = $\frac{1}{2} \times 2\pi r \times r $"),
        )
        text6 = VGroup(
            Text("即圆的面积为：", font_size=36),
            Tex(r"S = $\pi r^2 $"),
        )
        text5.arrange(RIGHT, buff=SMALL_BUFF)
        text5.move_to(UP * 3)  #将文字移动到圆的上方，可根据需要调整
        text6.arrange(RIGHT, buff=SMALL_BUFF)
        text6.move_to(DOWN * 3)  #将文字移动到圆的上方，可根据需要调整
        self.play(Write(text5,text6), run_time=1)
        self.wait(2)
        '''
        text5 = VGroup(
            Text("这个类等腰三角形的面积为：", font_size=36),
